...
const std::string Command::GetText()
{
	return _tokens.size() ? _tokens[0] : std::string();
}
...
const std::string Command::GetArg(int argnum)
{
	return (int)_tokens.size() < argnum + 1 ? std::string() : _tokens[argnum + 1];
}
